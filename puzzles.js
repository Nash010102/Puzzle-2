...
const Puzzles = {
  ...
  submitWire(p, inner) {
    ...
    if (allCorrect) {
      ...
      feedback.textContent = `✗ ${errorMsg}Re-examine the documentation.`;
      feedback.className = 'puzzle-feedback wrong';
      feedback.style.display = 'block';
    }
  },
  ...
  // ────────────────────────────────────────
  // BINARY DECODE — ACCESS TERMINAL
  // ────────────────────────────────────────
  renderBinaryDecode(p, container) {
    const inner = document.createElement('div');
    inner.style.cssText = 'font-family:var(--mono);padding:.5rem 0';
    inner.innerHTML = `
      <div style="color:var(--red-bright);font-size:1.2rem;letter-spacing:.18em;text-align:center;margin-bottom:.5rem;font-weight:700">■ ACCESS DENIED ■</div>
      <div style="color:var(--text-dim);font-size:.8rem;text-align:center;margin-bottom:.6rem;line-height:1.8">
        Authentication requires machine language.<br>
        <span style="color:var(--text-faint)">Human language is inefficient.</span>
      </div>
      <div style="color:var(--text-faint);font-size:.65rem;text-align:center;letter-spacing:.04em;margin-bottom:.9rem">▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓</div>
      <div style="font-size:1.1rem;letter-spacing:.15em;line-height:2.4;text-align:center;color:var(--gold)">
        ${p.bytes.map(b => `<div>${b.binary}</div>`).join('')}
      </div>
      <div style="color:var(--text-faint);font-size:.65rem;text-align:center;letter-spacing:.04em;margin:.8rem 0 1.1rem">▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓</div>
      <div style="display:flex;align-items:center;gap:.5rem;margin-bottom:.75rem">
        <span style="color:var(--gold);font-size:.82rem;white-space:nowrap;flex-shrink:0">DECRYPT &gt;</span>
        <input id="binary-word-input" maxlength="20" autocomplete="off" spellcheck="false"
          style="background:transparent;border:none;border-bottom:1px solid var(--gold);color:var(--gold);
                 font-family:var(--mono);font-size:.9rem;letter-spacing:.15em;outline:none;
                 width:100%;padding:.2rem 0;text-transform:uppercase">
      </div>
      <button class="btn-submit" id="btn-binary-submit" style="width:100%;letter-spacing:.1em">AUTHENTICATE</button>
      <div id="binary-feedback" class="puzzle-feedback" style="display:none;margin-top:.75rem"></div>`;
    container.appendChild(inner);

    const input = inner.querySelector('#binary-word-input');
    const submitBtn = inner.querySelector('#btn-binary-submit');
    ...
    const doSubmit = () => {
      ...
      if (!word) return;
      fb.style.display = 'block';
      if (word === p.finalAnswer) {
        fb.className = 'puzzle-feedback correct';
        fb.textContent = '✓ IDENTITY CONFIRMED. System access granted.';
        submitBtn.disabled = true;
        input.disabled = true;
        setTimeout(() => Game.revealHint(p.id), 900);
      } else {
        fb.className = 'puzzle-feedback wrong';
        fb.textContent = '✗ AUTHENTICATION FAILED.';
        input.value = '';
        input.focus();
      }
    };
