...
const Game = {
  ...
  showDetectiveBoard() {
    ...
    const poolHtml = unplaced.length > 0
      ? unplaced.map(card => `
          <div class="ev-card" draggable="true" data-card="${card.id}" id="evcard-${card.id}" onclick="Game.selectBoardCard('${card.id}')">
            <span class="ev-card-icon">${card.icon}</span>
            <div class="ev-card-name">${card.name}</div>
            <div class="ev-card-desc">${card.desc}</div>
            <div style="font-size:.68rem;color:var(--text-faint);margin-top:.3rem">Click to select, then click a suspect</div>
          </div>`).join('')
      : '<div style="font-size:.8rem;color:var(--text-faint);font-style:italic">All cards placed</div>';

    // Highlight selected card
    const selId = this.state.selectedBoardCard;
    ...
    const suspectCols = p.suspects.map(suspect => {
      ...
      const cardHtml = cardsHere.map(card => {
        return `<div class="board-card" data-card="${card.id}" title="Click to remove">
      ...
      return `
        <div class="suspect-col ${suspect.id==='ruledOut'?'ruled-out-col':''}"
          ...
          onclick="Game.placeSelectedCard('${suspect.id}')">
    ...
    const allPlaced = totalCards > 0 && placed === totalCards;
    const accuseBtn = allPlaced
      ...
      : `<div style="text-align:center;font-family:var(--mono);font-size:.78rem;color:var(--text-faint);margin-top:1rem">
           ${placed}/${totalCards} cards placed
           <br><em>Place all evidence before making your accusation.</em>
         </div>`;
    ...
    app.innerHTML = `
      <div id="screen-board">
        <div class="board-header">
          ...
          <p>Build your case. Connect each piece of evidence to the suspect you believe it concerns.<br>Mark planted or irrelevant evidence as "Ruled Out / Explained".</p>
        </div>
  ...
  selectBoardCard(cardId) {
    if (this.state.selectedBoardCard === cardId) {
      this.state.selectedBoardCard = null;
    } else {
      this.state.selectedBoardCard = cardId;
    }
    // Highlight selected card visually
    document.querySelectorAll('.ev-card').forEach(el => {
      el.classList.toggle('selected', el.dataset.card === this.state.selectedBoardCard);
    });
  },

  placeSelectedCard(suspectId) {
    const cardId = this.state.selectedBoardCard;
    if (!cardId) return; // nothing selected
    const p = PUZZLES['detective_board'];
...
[ Police sirens. You step outside. You survived. ]</div>
  ...
  resetGame() {
    this.state = {
      ...
      boardPlacements: {}, accusation: null, selectedBoardCard: null
    };
    this.renderIntro();
  },
